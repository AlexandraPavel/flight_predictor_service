...
sns.set()
model = open('/output/model_3.pkl','rb')
...
for col in col_list:
    ...
    counter += 1
y_prediction = forest.predict(dataframe_logic)
counter = 0
for col in col_list:
...
for col in col_list:
    ...
    counter2 += 1
y_return_prediction = forest.predict(dataframe_return_logic)
    ...
    if (max_ret_inq - min_inq) > days_diff:
        for elem in day_list:
            if int(int(elem) + days_diff) in return_day_list:
                elem_found = 1
                print("We believe the best day for purchasing both of the tickets would be: " + (d1 - timedelta(days = elem)).strftime('%d %B %Y'))
                break
    if elem_found == 1:
        break
if elem_found == 0:
    print("We believe the best day for purchasing both of the tickets would be: " + (d1 - timedelta(days=dataframe_logic.iloc[:1]["Enquiry_days_before"])).strftime('%d %B %Y'))